...
- 사용자는 화면을 보고 `Host -> Agent -> R1 -> R2 -> Monitor` 흐름에서 지금 어떤 일이 일어나는지 따라갈 수 있어야 한다.
...
- Host Simulator, Local Agent, R1, R2, Monitor node 카드를 모두 보여준다.
- node 카드는 preview처럼 diagram canvas 안에서 absolute position으로 배치한다.
- `Host -> Agent -> R1 -> R2 -> Monitor` topology는 별도 상단 strip이 아니라 diagram canvas 안의 SVG path와 label로 표현한다.
