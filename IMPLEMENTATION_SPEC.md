...
#### Host Simulator
- 주기적으로 host 관측값을 생성한다.
- 정상 관측값과 fault 주입에 따른 비정상 관측값을 시뮬레이션한다.
- 네트워크 EVENT를 직접 forwarding 하지 않는다.

#### Local Agent
- Host Simulator의 원천 관측값을 읽는다.
- 정상 host 관측값 변화와 임계치 초과 fault를 판단한다.
...
#### 책임
- 현재 시뮬레이션된 host 관측값 유지
- 시간이나 fault 주입에 따라 정상 또는 비정상 관측값 생성
...
#### 최소 지원 fault injection 입력
- 아래 값은 Host가 작성하는 fault 상태가 아니라, Host 관측값을 비정상으로 만들기 위한 제어 입력이다.
- `CPU_SPIKE`
- `SERVICE_DOWN`
- `LATENCY_HIGH`
...
#### 이벤트 생성 규칙
- 의미 있는 관측값 변화 또는 임계치 초과 때만 생성
- 같은 이벤트를 매 tick마다 spam 하지 않음
- `payload.fault_mode`는 호환성을 위해 유지하지만 Local Agent가 판단한 `event_type` 또는 정상 상태의 `NORMAL`에서 작성한다.
- `LATENCY_HIGH`는 Host의 `latency_state`가 아니라 `latency_ms >= 200`으로 판단한다.
