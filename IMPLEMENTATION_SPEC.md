...
- Host / Agent / R1 / R2 / Monitor data path에 새 peer로 참여하지 않는다.
...
#### 제어 경로
`Controller/UI -> Host Simulator, Local Agent, Relay R1, Relay R2, Monitor`
...
- relay processing delay는 기본 `1.5`초이며, `delay r1|r2 <sec>` 명령으로 조정할 수 있다.
...
로컬 기본 viewer에서는 Host / Agent / R1 / R2와 같은 supervisor-managed child role로 기동하고 종료한다.
...
- `--focus-node`는 시작 시 초기 focused mode를 정하는 seed이며, 실행 중에는 같은 controller/UI의 `viewer>` 프롬프트에서 `focus host|agent|r1|r2|monitor`로 대상을 바꾼다.
...
- R1, R2, Monitor는 각각 처리된 ID cache를 유지한다.
